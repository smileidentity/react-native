//
//  SmileIDSmartSelfieEnrollmentEnhancedView.swift
//  SmileIDShared
//
...
/// Shared SmartSelfieEnrollmentEnhanced view that uses SmileIDViewConfig
@available(iOS 13.0, *)
public struct SmileIDSmartSelfieEnrollmentEnhancedView: View {
        ...
        Group {
            ...
            if !config.validateSelfieProperties() {
                ...
                Text("")
                    .onAppear {
                        let error = NSError(
                            domain: "SmileIDSmartSelfieEnrollmentEnhancedView",
                            code: -1,
                            userInfo: [NSLocalizedDescriptionKey: "Required properties for SmartSelfieEnrollmentEnhanced are missing"]
                        )
                        onResult(.withError(cause: error))
                    }
            } else {
                // Configuration is valid, show the SmartSelfieEnrollmentEnhanced screen
                SmartSelfieEnrollmentEnhancedContent(
                    config: config,
                    onResult: onResult
                )
            }
...
@available(iOS 13.0, *)
private struct SmartSelfieEnrollmentEnhancedContent: View {
    let config: SmileIDViewConfig
    let onResult: (SmileIDSharedResult<Any>) -> Void
