//
//  SmileIDSmartSelfieAuthEnhancedView.swift
//  SmileIDShared
//
...
/// Shared SmartSelfieAuthenticationEnhanced view that uses SmileIDViewConfig
@available(iOS 13.0, *)
public struct SmileIDSmartSelfieAuthEnhancedView: View {
        ...
        Group {
            ...
            if !config.validateSelfieProperties() {
                ...
                Text("")
                    .onAppear {
                        let error = NSError(
                            domain: "SmileIDSmartSelfieAuthEnhancedView",
                            code: -1,
                            userInfo: [NSLocalizedDescriptionKey: "Required properties for SmartSelfieAuthenticationEnhanced are missing"]
                        )
                        onResult(.withError(cause: error))
                    }
            } else {
                // Configuration is valid, show the SmartSelfieAuthenticationEnhanced screen
                SmartSelfieAuthEnhancedContent(
                    config: config,
                    onResult: onResult
                )
            }
...
@available(iOS 13.0, *)
private struct SmartSelfieAuthEnhancedContent: View {
    let config: SmileIDViewConfig
    let onResult: (SmileIDSharedResult<Any>) -> Void
