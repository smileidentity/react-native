...
class SmileIDEnhancedDocumentVerificationViewManager: SmileIDBaseViewManager {
    ...
    override static func moduleName() -> String! {
        return "SmileIDEnhancedDocumentVerificationView"
    }
    ...
    // Override setConfig to use the fromEnhancedDocumentMap validation
    @objc override func setConfig(_ view: UIView, config configDict: NSDictionary?) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            do {
                // Use the fromEnhancedDocumentMap method to create config with validation
                self.config = try SmileIDViewConfig.fromEnhancedDocumentMap(configDict as! [String: Any])
